...
{/* TODO: Add workflow phases screenshot when ready
     Image path: /images/screenshots/agent-workflow-phases.png
     See /images/screenshots/README.md for requirements

## Visual Workflow Overview

![Compyle's three-phase agent workflow showing research, planning, and implementation stages](/images/screenshots/agent-workflow-phases.png)

*Compyle's collaborative workflow keeps you in control at every stage.*
...
<Tip title="See Planning in Action">
  Want to see the planning phase visually? Check out the [interactive planning demo](https://www.compyle.ai/planning.html) on our homepage, or watch the [video walkthrough](https://youtu.be/iNQ8G4k03gc) to see the three-phase workflow in action with real examples.
</Tip>
...
```mermaid
graph TD
    A[Task Created] --> B[Research Phase]
    B --> C[Compyle analyzes codebase]
    C --> D[research.md created]
    D --> E{Review research artifact}
    E -->|Feedback provided| B
    E -->|Looks good| F[Planning Phase]
    F --> G[Compyle asks clarifying questions]
    G --> H[You answer questions]
    H --> I[planning.md created]
    I --> J{Review planning spec}
    J -->|Adjustments needed| G
    J -->|Approved| K[Implementation Phase]
    K --> L[Compyle builds step-by-step]
    L --> M{Validation checks}
    M -->|Issue found| N[Agent asks for guidance]
    N --> L
    M -->|All checks pass| O[PR Created]
    O --> P[You review & test]
    P --> Q{Approve?}
    Q -->|Changes needed| L
    Q -->|Approved| R[Merge to target branch]
